 ...
 *
 * In the fvmc_tesselation_t structure, the encoding[] array corresponds
...
#define _ENCODING_BITS (sizeof(fvmc_tesselation_encoding_t)*8/3)
...
typedef unsigned fvmc_tesselation_encoding_t;

/*----------------------------------------------------------------------------
 * Structure defining a tesselation of a mesh section.
 *----------------------------------------------------------------------------*/

struct _fvmc_tesselation_t {
  ...
  int               n_sub_types;         /* Number of sub-element types
                                            (currently max 2) */
  fvmc_element_t     sub_type[2];         /* Sub-element types */

  fvmc_lnum_t        n_sub_max[2];        /* Maximum number of sub-elements of
                                            each type per element */
  fvmc_lnum_t        n_sub_max_glob[2];   /* Global maximum number of
                                            sub-elements of each type
                                            per element */
  fvmc_lnum_t        n_sub[2];            /* Number of sub-elements of
                                            each type */
  fvmc_gnum_t        n_sub_glob[2];       /* Global number of sub-elements
                                            of each type */

  const fvmc_tesselation_encoding_t  *encoding;    /* Compact tesselation
                                                     encoding array
                                                     (see notes above) */

  fvmc_tesselation_encoding_t        *_encoding;   /* encoding if owner,
                                                     NULL if shared */

  const fvmc_lnum_t  *sub_elt_index[2];   /* index of sub-elements of each
                                            given type associated with
                                            each element (0 to n-1); */
  fvmc_lnum_t        *_sub_elt_index[2];  /* sub_elt_index if owner,
                                            NULL if shared */
...
static inline void
_added_vertex_coords(const fvmc_tesselation_t  *ts,
                     fvmc_coord_t               vertex_coords[3],
                     int                      *n_vertices_tot,
                     fvmc_lnum_t                element_id)
{
  ...
  fvmc_lnum_t _n_vertices_tot = 0;
  fvmc_coord_t cell_center[3] = {0., 0., 0.};
  fvmc_coord_t cell_center_divisor = 0.;
  ...
  for (i = ts->face_index[element_id];
    ...
    fvmc_coord_t sign, v1[3], v2[3];

    fvmc_coord_t vertices_center[3] = {0., 0., 0.};
    fvmc_coord_t face_center[3] = {0., 0., 0.};
    fvmc_coord_t face_normal[3] = {0., 0., 0.};
    fvmc_coord_t triangle_center[3] = {0., 0., 0.};
    fvmc_coord_t triangle_normal[3] = {0., 0., 0.};
    fvmc_coord_t face_surface = 0.;
    fvmc_coord_t triangle_surface = 0.;
    const fvmc_coord_t *current_coords = NULL;

    face_id = FVMC_ABS(ts->face_num[i]) - 1;
    ...
    for (k = 0; k < 3; k++)
      vertices_center[k] /= (fvmc_coord_t)n_vertices;
              ...
              double  *restrict x)
{
  ...
  for (i = 0; i < 4-1; i++) {
    ...
    k_pivot = i;
    abs_pivot = FVMC_ABS(_a[i][i]);

    for (k = i+1; k < 4; k++) {
      abs_a_ki = FVMC_ABS(_a[k][i]);
...
static inline void
_polyhedron_vertices(const fvmc_tesselation_t   *ts,
                     fvmc_lnum_t                 heap[],
                     fvmc_lnum_t                 vertex_list[],
                     int                       *vertex_list_size,
                     fvmc_lnum_t                 element_id)
{
  fvmc_lnum_t n_vertices;
  fvmc_lnum_t i, j, face_id, vertex_id;
...
static void
_vertex_field_of_real_values(const fvmc_tesselation_t  *this_tesselation,
                             int                       src_dim,
                             int                       src_dim_shift,
                             int                       dest_dim,
                             ...
                             void               *const dest_data)
{
  ...
  fvmc_lnum_t i, j, parent_id, src_id, vertex_id;

  int _src_dim_shift = src_dim_shift;
  int max_list_size = 128;

  fvmc_lnum_t _heap[128];
  fvmc_lnum_t _vertex_list[128];
  fvmc_lnum_t *heap = _heap;
  fvmc_lnum_t *vertex_list = _vertex_list;

  const fvmc_tesselation_t *ts = this_tesselation;
  const fvmc_coord_t *current_coords = NULL;

  if (ts->type != FVMC_CELL_POLY)
    return;
  ...
  for (i = start_id; i < end_id; i++) {
    ...
    for (j = 0; j < dest_dim; j++) { /* Loop on destination dimension */
      ...
      if (n_vertices_tot > max_list_size) {
        ...
        BFTC_REALLOC(heap, max_list_size, fvmc_lnum_t);
        BFTC_REALLOC(vertex_list, max_list_size, fvmc_lnum_t);
      }
...
static void
_init_decoding_mask(fvmc_tesselation_encoding_t decoding_mask[3])
{
  unsigned i;
 ...
 * referred to by an fvmc_tesselation_t structure. Quadrangles are not
 * tesselated.
 *
 * parameters:
...
static void
_tesselate_polygons(fvmc_tesselation_t  *this_tesselation,
                    int                 dim,
...
{
  int type_id;
  fvmc_lnum_t n_vertices, n_triangles, n_quads, n_elements;
  fvmc_lnum_t n_vertices_max, n_triangles_max;
  fvmc_lnum_t i, j, k, vertex_id, encoding_id;
  fvmc_tesselation_encoding_t encoding_sub[3];

  fvmc_gnum_t n_g_elements_tot[2] = {0, 0}; /* Global new elements count */
  fvmc_lnum_t n_elements_tot[2] = {0, 0}; /* New triangles/quadrangles */
  fvmc_lnum_t n_g_elements_max[2] = {0, 0}; /* Global max triangles/quadrangles */
  fvmc_lnum_t n_elements_max[2] = {0, 0}; /* Max triangles/quadrangles */
  fvmc_lnum_t *triangle_vertices = NULL;
  fvmc_triangulate_state_t *state = NULL;

  fvmc_tesselation_t *ts = this_tesselation;
  ...
  if (n_vertices_max > 2<<_ENCODING_BITS)
    bftc_error(__FILE__, __LINE__, 0,
              ...
              sizeof(fvmc_tesselation_encoding_t)*8,
  ...
  if (ts->_encoding != NULL)
    BFTC_FREE(ts->_encoding);

  /* Allocate memory and state variables */
  /*-------------------------------------*/

  if (n_vertices_max > 4) {
    BFTC_MALLOC(ts->_encoding,
               ts->vertex_index[n_elements] - n_elements*2,
               fvmc_tesselation_encoding_t);
    ts->encoding = ts->_encoding;
    BFTC_MALLOC(triangle_vertices, (n_vertices_max - 2) * 3, fvmc_lnum_t);
  ...
  for (i = 0 ; i < n_elements ; i++) {
    ...
    if (n_vertices > 4) {
      ...
      for (j = 0; j < n_triangles; j++) {
        ...
        for (k = 0; k < 3; k++)
          encoding_sub[k]
            = (   ((fvmc_tesselation_encoding_t)(triangle_vertices[j*3 + k] - 1))
               << (_ENCODING_BITS * k));
  ...
  fvmc_parall_counter(n_g_elements_tot, 2);
  fvmc_parall_counter_max(n_g_elements_max, 2);

  for (type_id = 0; type_id < 2; type_id++) {
    if (n_g_elements_tot[type_id] > 0) {
      if (type_id == 0)
        ts->sub_type[ts->n_sub_types] = FVMC_FACE_TRIA;
      else
        ts->sub_type[ts->n_sub_types] = FVMC_FACE_QUAD;
 ...
 * to by an fvmc_tesselation_t structure.
 *
 * parameters:
 *   this_tesselation   <-> partially initialized tesselation structure
...
static void
_count_and_index_sub_polygons(fvmc_tesselation_t  *this_tesselation,
                              _Bool               global_count)
{
  int sub_type_id, type_id;
  fvmc_lnum_t n_vertices, n_triangles, n_elements;
  fvmc_lnum_t i, j, encoding_id;
  ...
  for (sub_type_id = 0; sub_type_id < ts->n_sub_types; sub_type_id++) {
    ...
    BFTC_MALLOC(ts->_sub_elt_index[sub_type_id], n_elements + 1, fvmc_lnum_t);
  ...
  if (global_count == true && ts->global_element_num != NULL) {
    ...
    for (sub_type_id = 0; sub_type_id < ts->n_sub_types; sub_type_id++) {
      if (ts->_sub_elt_index[sub_type_id] != NULL) {
        fvmc_lnum_t * _n_sub_elements = ts->_sub_elt_index[sub_type_id] + 1;
        if (n_elements == 0) _n_sub_elements = NULL;
        ts->n_sub_glob[sub_type_id]
          = fvmc_io_num_global_sub_size(ts->global_element_num,
                                       _n_sub_elements);
      }
      else
  ...
  for (sub_type_id = 0; sub_type_id < ts->n_sub_types; sub_type_id++) {
    ...
    fvmc_lnum_t *sub_elt_index = ts->_sub_elt_index[sub_type_id];
    for (i = 0 ; i < n_elements ; i++)
 ...
 * polyhedra of a mesh section referred to by an fvmc_tesselation_t
...
static void
_count_and_index_sub_polyhedra(fvmc_tesselation_t  *this_tesselation,
                               fvmc_lnum_t         *error_count,
                               _Bool               global_count)
{
  int sub_type_id, type_id;
  fvmc_lnum_t n_vertices, n_triangles, n_elements;
  fvmc_lnum_t n_tetras, n_pyrams;
  fvmc_lnum_t i, j, k, face_id, encoding_id;

  fvmc_gnum_t n_g_elements_tot[2] = {0, 0}; /* Global new elements count */
  fvmc_lnum_t n_elements_tot[2] = {0, 0}; /* New tetrahedra/pyramids */
  fvmc_lnum_t *n_sub_elements[2] = {NULL, NULL};
  fvmc_lnum_t n_g_elements_max[2] = {0, 0}; /* Global max tetrahedra/pyramids */
  fvmc_lnum_t n_elements_max[2] = {0, 0}; /* Max tetrahedra/pyramids */
  ...
  for (i = 0 ; i < n_elements ; i++) {
    ...
    for (j = ts->face_index[i];     /* Loop on element faces */
      ...
      face_id = FVMC_ABS(ts->face_num[j]) - 1;
  ...
  for (type_id = 0; type_id < 2; type_id++) {
    if (n_g_elements_tot[type_id] > 0) {
      if (type_id == 0)
        ts->sub_type[ts->n_sub_types] = FVMC_CELL_TETRA;
      else
        ts->sub_type[ts->n_sub_types] = FVMC_CELL_PYRAM;
      ts->n_sub_max[ts->n_sub_types] = n_elements_max[type_id];
      ts->n_sub_max_glob[ts->n_sub_types] = n_g_elements_max[type_id];
      ts->n_sub[ts->n_sub_types] = n_elements_tot[type_id];
      ts->n_sub_glob[ts->n_sub_types] = n_elements_tot[type_id];
...
static void
_expand_limit_g(const fvmc_tesselation_t  *this_tesselation,
                fvmc_lnum_t                end_id,
                ...
                MPI_Comm                  comm)
{
  ...
  const fvmc_gnum_t *global_element_num
    = fvmc_io_num_get_global_num(this_tesselation->global_element_num);
  ...
  MPI_Allreduce(&local_max, &global_max, 1, FVMC_MPI_GNUM, MPI_MIN, comm);
...
static fvmc_lnum_t
_decode_polygons_tesselation_g(const fvmc_tesselation_t  *this_tesselation,
...
{
  fvmc_lnum_t n_vertices;
  fvmc_lnum_t i, j, k, l, vertex_id, encoding_id;
  fvmc_lnum_t tv[3];
  fvmc_tesselation_encoding_t decoding_mask[3] = {0, 0, 0};

  fvmc_lnum_t n_sub_tot = 0;
  const fvmc_tesselation_t *ts = this_tesselation;
  fvmc_lnum_t retval = start_id;

  const fvmc_gnum_t *_global_vertex_num
                      = fvmc_io_num_get_global_num(global_vertex_num);
  const fvmc_gnum_t *global_element_num
    = fvmc_io_num_get_global_num(ts->global_element_num);
  ...
  for (i = 0, j = start_id ;
    ...
    if (   connect_type == FVMC_FACE_TRIA
    ...
    else if (   (connect_type == FVMC_FACE_TRIA && n_vertices == 3)
             || (connect_type == FVMC_FACE_QUAD && n_vertices == 4)) {
...
static fvmc_lnum_t
_decode_polygons_tesselation_l(const fvmc_tesselation_t  *this_tesselation,
                               fvmc_element_t             connect_type,
                               fvmc_lnum_t                start_id,
                               fvmc_lnum_t                buffer_limit,
...
static fvmc_lnum_t
_decode_polyhedra_tesselation_g(const fvmc_tesselation_t  *this_tesselation,
                                fvmc_element_t             connect_type,
                                fvmc_lnum_t                start_id,
                                fvmc_lnum_t                buffer_limit,
                                fvmc_gnum_t                global_num_end,
                                fvmc_gnum_t                extra_vertex_base_num,
                                const fvmc_io_num_t       *global_vertex_num,
                                fvmc_gnum_t                vertex_num[])
{
  int orient;
  fvmc_lnum_t n_vertices;
  fvmc_lnum_t i, j, k, l, m, base_dest_id, face_id, vertex_id, encoding_id;
  fvmc_lnum_t tv[3];
  fvmc_tesselation_encoding_t decoding_mask[3] = {0, 0, 0};
  ...
  for (i = 0, j = start_id ;
    ...
    for (k = ts->face_index[j];     /* Loop on element faces */
      ...
      face_id = FVMC_ABS(ts->face_num[k]) - 1;
      ...
      if (   connect_type == FVMC_CELL_TETRA
          && n_vertices > 3 && ts->encoding != NULL) {
      ...
      /*-----------------------------------*/
      ...
      else if (   (connect_type == FVMC_CELL_TETRA && n_vertices == 3)
               || (connect_type == FVMC_CELL_PYRAM && n_vertices == 4)) {
        ...
        if (l == n_vertices - 2) {
          ...
          fvmc_lnum_t stride = n_vertices + 1;
...
static fvmc_lnum_t
_decode_polyhedra_tesselation_l(const fvmc_tesselation_t  *this_tesselation,
                                fvmc_element_t             connect_type,
                                fvmc_lnum_t                start_id,
                                fvmc_lnum_t                buffer_limit,
                                fvmc_lnum_t                extra_vertex_base_num,
                                fvmc_lnum_t                vertex_num[])
{
  int orient;
  fvmc_lnum_t n_vertices;
...

{
  ...
  fvmc_tesselation_t  *this_tesselation;
  ...
  BFTC_MALLOC(this_tesselation, 1, fvmc_tesselation_t);
    ...
    if (element_type != FVMC_FACE_POLY)
      bftc_error(__FILE__, __LINE__, 0,
                _("Incoherent connectivity for tesselation:\n"
                  ...
                  "but element type != FVMC_FACE_POLY"));
  }

  /* Compute number of polyhedron faces */

  if (n_elements > 0 && face_index != NULL) {
    fvmc_lnum_t  j, k, face_id;
    fvmc_lnum_t  max_face_id = 0;
    for (j = 0; j < n_elements; j++) {
      for (k = face_index[j]; k < face_index[j+1]; k++) {
        face_id = FVMC_ABS(face_num[k]) - 1;
  ...
  for (i = 0; i < FVMC_TESSELATION_N_SUB_TYPES_MAX; i++) {
    ...
    this_tesselation->sub_type[i] = FVMC_N_ELEMENT_TYPES;
  }
...
fvmc_tesselation_destroy(fvmc_tesselation_t  * this_tesselation)
{
  ...
  if (this_tesselation->_encoding != NULL)
    BFTC_FREE(this_tesselation->_encoding);

  for (i = 0; i < this_tesselation->n_sub_types; i++) {
    if (this_tesselation->_sub_elt_index[i] != NULL)
      BFTC_FREE(this_tesselation->_sub_elt_index[i]);
  }
  BFTC_FREE(this_tesselation);
                     ...
                     fvmc_lnum_t         *error_count)
{
  ...
  default:
      bftc_error(__FILE__, __LINE__, 0,
                ...
                fvmc_elements_type_name[this_tesselation->type]);
...
fvmc_tesselation_n_g_vertices_add(const fvmc_tesselation_t  *this_tesselation)
{
  ...
  if (this_tesselation->type == FVMC_CELL_POLY) {

    if (this_tesselation->global_element_num != NULL)
      retval = fvmc_io_num_get_global_count(this_tesselation->global_element_num);
    else
      retval = this_tesselation->n_elements;
...
fvmc_tesselation_sub_type(const fvmc_tesselation_t  *this_tesselation,
                         int                       sub_type_id)
{
  fvmc_element_t retval = FVMC_N_ELEMENT_TYPES;

  if (this_tesselation == NULL)
    retval = FVMC_N_ELEMENT_TYPES;
  else {
...
{
  const fvmc_io_num_t *retval = NULL;
...
{
  fvmc_lnum_t i;
  fvmc_lnum_t buffer_size = buffer_limit * stride;

  const fvmc_gnum_t *global_element_num
    = fvmc_io_num_get_global_num(this_tesselation->global_element_num);

  const fvmc_lnum_t *sub_element_idx
    = fvmc_tesselation_sub_elt_index(this_tesselation, connect_type);
...
{
  int id;
  fvmc_lnum_t  i, j, k, n_sub;
  size_t  l;
  char  *src, *dest;

  const fvmc_lnum_t *sub_elt_index = NULL;
                              ...
                              void               *const dest_data)
{
  ...
  if (   (src_datatype != FVMC_DOUBLE && src_datatype != FVMC_FLOAT)
      || (dest_datatype != FVMC_DOUBLE && dest_datatype != FVMC_FLOAT)) {

    size_t data_size_c =    (end_id - start_id)
                          * (dest_dim * fvmc_datatype_size[dest_datatype]);
...
{
  int  i;
  fvmc_lnum_t  n_elements, j, k;
  const fvmc_lnum_t  *idx;
  ...
  for (i = 0; i < this_tesselation->n_sub_types; i++) {
    bftc_printf("Maximum local number of resulting %s per element: %ld\n",
               fvmc_elements_type_name[this_tesselation->sub_type[i]],
               (long)(this_tesselation->n_sub_max[i]));
  }

  for (i = 0; i < this_tesselation->n_sub_types; i++) {
    bftc_printf("Maximum global number of resulting %s per element: %ld\n",
               fvmc_elements_type_name[this_tesselation->sub_type[i]],
  ...
  for (i = 0; i < this_tesselation->n_sub_types; i++) {
    bftc_printf("Local number of resulting %s: %ld\n",
               fvmc_elements_type_name[this_tesselation->sub_type[i]],
               (long)(this_tesselation->n_sub[i]));
  }

  for (i = 0; i < this_tesselation->n_sub_types; i++) {
    bftc_printf("Global number of resulting %s: %lu\n",
               fvmc_elements_type_name[this_tesselation->sub_type[i]],
  ...
  for (i = 0; i < this_tesselation->n_sub_types; i++) {
    if (this_tesselation->sub_elt_index[i] != NULL)
      bftc_printf("  _sub_elt_index[%s]: %p\n",
                 fvmc_elements_type_name[this_tesselation->sub_type[i]],
  ...
  if (this_tesselation->encoding != NULL) {
    ...
    if (this_tesselation->type != FVMC_FACE_QUAD) {
      ...
      for (j = 0; j < n_elements; j++) {
        _DECODE_TRIANGLE_VERTICES(tv,
                                  ...
                                  decoding_mask);
        bftc_printf("%10d (idx = %10d) %10d %10d %10d\n",
    ...
    else { /* if (this_tesselation->type != FVMC_FACE_QUAD) */
