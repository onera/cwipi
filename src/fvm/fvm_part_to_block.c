                        ...
                        MPI_Comm              comm)
{
  ...
  for (j = 0; j < d->n_part_ents; j++) {
    ...
    send_block_id[d->send_displ[send_rank]] = (fvm_lnum_t) (g_ent_id % block_size);
                     ...
                     MPI_Comm              comm)
{
  ...
  for (j = 0; j < d->n_part_ents; j++)
    send_block_id[j] = (fvm_lnum_t) global_ent_num[j] -1;
...
static void
_copy_data_alltoallv(fvm_part_to_block_t   *d,
                     fvm_datatype_t         datatype,
                     int                   *pv_idx,
                     const void            *part_values,
                     int                   *bv_idx,
                     void                  *block_values)
{
  ...
  BFT_MALLOC(send_strides_buf, d->n_part_ents, int);
  ...
  BFT_MALLOC(recv_strides_buf, n_recv_ents, int);
  ...
  BFT_FREE(send_strides_buf);

  /* Compute send and receive dimensions */
  /* ----------------------------------- */


  BFT_MALLOC(_send_count, n_ranks, int);
  BFT_MALLOC(_send_displ, n_ranks, int);
  ...
  BFT_MALLOC(_recv_count, n_ranks, int);
  BFT_MALLOC(_recv_displ, n_ranks, int);
  ...
  BFT_MALLOC(send_buf, sum_strides_size, unsigned char);
  ...
  BFT_FREE(recv_buf);
  BFT_FREE(send_buf);
  BFT_FREE(recv_strides_buf);
  BFT_FREE(_send_count);
  BFT_FREE(_send_displ);
  BFT_FREE(_recv_count);
  BFT_FREE(_recv_displ);
}
...
static void
_copy_data_gather(fvm_part_to_block_t   *d,
                  fvm_datatype_t         datatype,
                  int                   *pv_idx,
                  const void            *part_values,
                  int                   *bv_idx,
