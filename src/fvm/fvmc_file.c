...
struct _fvmc_file_t {
  ...
  char              *name;         /* File name */
  fvmc_file_mode_t    mode;         /* File mode */
  ...
  bftc_file_t        *sh;           /* Serial file handle */
...
#if defined(FVMC_HAVE_MPI_IO)
static fvmc_file_hints_t _default_semantics = FVMC_FILE_INDIVIDUAL_POINTERS;
#else
static fvmc_file_hints_t _default_semantics = 0;
#endif

/* Convert fvmc_file_seek to bftc_file_seek */

static bftc_file_seek_t _bftc_seek[3] = {BFTC_FILE_SEEK_SET,
                                       BFTC_FILE_SEEK_CUR,
                                       BFTC_FILE_SEEK_END};

#if defined(FVMC_HAVE_MPI_IO)

/* Convert fvmc_file_seek to MPI_File_seek */

static int _mpi_seek[3] = {MPI_SEEK_SET,
                           MPI_SEEK_CUR,
                           MPI_SEEK_END};

#endif /* defined(FVMC_HAVE_MPI_IO) */
...
static int
_file_open(fvmc_file_t       *f,
           fvmc_file_mode_t   mode)
{
  bftc_file_mode_t _mode;
  ...
  switch (f->mode) {
  case FVMC_FILE_MODE_APPEND:
    _mode = BFTC_FILE_MODE_APPEND;
    break;
  case FVMC_FILE_MODE_WRITE:
    _mode = BFTC_FILE_MODE_WRITE;
    break;
  default:
    assert(f->mode == FVMC_FILE_MODE_READ);
    _mode = BFTC_FILE_MODE_READ;
  }

  f->sh = bftc_file_open(f->name, _mode, BFTC_FILE_TYPE_BINARY);
...
static int
_file_close(fvmc_file_t  *f)
{
  int retval = 0;

  f->sh = bftc_file_free(f->sh);
...
static size_t
_file_read_block(fvmc_file_t  *f,
                 void        *buf,
                 size_t       size,
  ...
  if (f->comm != MPI_COMM_NULL) {
    ...
    if (f->rank == 0) {
      if (f->n_ranks < 64)
        ...
        BFTC_MALLOC(counts, f->n_ranks, int);
    }
    ...
    if (f->rank == 0) {
      ...
      for (dist_rank = 1; dist_rank < f->n_ranks; dist_rank++)
        _buf_size = FVMC_MAX(_buf_size, counts[dist_rank]);

      BFTC_MALLOC(_buf, _buf_size*size, unsigned char);
      ...
      for (dist_rank = 1; dist_rank < f->n_ranks; dist_rank++) {
        ...
        counts[dist_rank]
          = (int)bftc_file_read(_buf, size, (size_t)counts[dist_rank], f->sh);
...
static size_t
_file_write_block(fvmc_file_t  *f,
                  void        *buf,
                  size_t       size,
                  fvmc_gnum_t   global_num_start,
                  fvmc_gnum_t   global_num_end)
  ...
  if (f->comm != MPI_COMM_NULL) {
    ...
    if (f->rank == 0) {
      ...
      for (dist_rank = 1; dist_rank < f->n_ranks; dist_rank++) {
        ...
        rank_id = dist_rank;
        MPI_Send(&rank_id, 1, MPI_INT, dist_rank, FVMC_MPI_TAG, f->comm);
        ...
        counts[dist_rank]
          = (int)bftc_file_write(_buf, size, (size_t)counts[dist_rank], f->sh);
    ...
    /* Other ranks receive data from rank 0 */
      ...
      MPI_Recv(&rank_id, 1, MPI_INT, 0, FVMC_MPI_TAG, f->comm, &status);
               ...
               fvmc_file_mode_t   mode)
{
  ...
  if (f->mode == FVMC_FILE_MODE_APPEND)
    amode = MPI_MODE_WRONLY | MPI_MODE_APPEND;

  else if (f->mode == FVMC_FILE_MODE_WRITE)
    amode = MPI_MODE_WRONLY | MPI_MODE_CREATE;

  else if (f->mode == FVMC_FILE_MODE_READ)
...
static size_t
_mpi_file_read_block_eo(fvmc_file_t  *f,
                        void        *buf,
                        size_t       size,
                        fvmc_gnum_t   global_num_start,
                        fvmc_gnum_t   global_num_end)
{
  fvmc_gnum_t global_num_end_last = global_num_end;
  size_t retval = 0;
  ...
  MPI_Bcast(&global_num_end_last, 1, FVMC_MPI_GNUM, f->n_ranks-1, f->comm);
...
static size_t
_mpi_file_read_block_ip(fvmc_file_t  *f,
                        void        *buf,
                        size_t       size,
                        fvmc_gnum_t   global_num_start,
                        fvmc_gnum_t   global_num_end)
...
static size_t
_mpi_file_write_block_eo(fvmc_file_t  *f,
                         void        *buf,
                         size_t       size,
                         fvmc_gnum_t   global_num_start,
                         fvmc_gnum_t   global_num_end)
...
static size_t
_mpi_file_write_block_ip(fvmc_file_t  *f,
                         void        *buf,
                         size_t       size,
                         fvmc_gnum_t   global_num_start,
                         fvmc_gnum_t   global_num_end)
...
#endif
{
  int errcode = 0;
  fvmc_file_t * f = NULL;
  fvmc_file_hints_t _hints = _default_semantics;

  BFTC_MALLOC(f, 1, fvmc_file_t);
  ...
  f->mode = mode;
  f->semantics = FVMC_FILE_NO_MPI_IO;
  f->rank = 0;
  f->n_ranks = 1;
...
#if defined(FVMC_HAVE_MPI_IO)
  if (   f->comm != MPI_COMM_NULL
      && !(_hints & FVMC_FILE_NO_MPI_IO)) {
    int positioning_mask = (  FVMC_FILE_EXPLICIT_OFFSETS
                            | FVMC_FILE_INDIVIDUAL_POINTERS);
    if (_hints & positioning_mask)
      f->semantics = hints & positioning_mask;
    else
      f->semantics = FVMC_FILE_INDIVIDUAL_POINTERS;
    f->semantics = f->semantics | (_hints & FVMC_FILE_NO_PREDISTRIBUTE);
  }
#endif
  ...
  if ((f->semantics & FVMC_FILE_NO_MPI_IO) && f->rank == 0)
    errcode = _file_open(f, f->mode);

#if defined(FVMC_HAVE_MPI_IO)
  else if (!(f->semantics & FVMC_FILE_NO_MPI_IO))
                     ...
                     size_t       ni)
{
...
#if defined(FVMC_HAVE_MPI)
  {
    if ((f->semantics & FVMC_FILE_NO_MPI_IO) && f->comm != MPI_COMM_NULL) {
      long _retval = retval;
      ...
      if (f->semantics & FVMC_FILE_EXPLICIT_OFFSETS) {
      ...
      else if (f->semantics & FVMC_FILE_INDIVIDUAL_POINTERS) {
        MPI_Datatype file_type;
        MPI_Aint disps[1];
        int lengths[1];
        char datarep[] = "native";
  ...
  if (f->swap_endian == true && size > 1)
    bftc_file_swap_endian(buf, buf, size, retval);
                      ...
                      size_t       ni)
{
  ...
  if (   f->rank == 0
    ...
    if (size*ni > sizeof(_copybuf))
      BFTC_MALLOC(copybuf, size*ni, unsigned char);
    memcpy(copybuf, buf, size*ni);

    if (f->swap_endian == true && size > 1)
      bftc_file_swap_endian(copybuf, copybuf, size, ni);

    _buf = copybuf;
  }

  if ((f->semantics & FVMC_FILE_NO_MPI_IO) && f->sh != NULL) {
    retval = bftc_file_write(_buf,
                            size,
                            ni,
                            f->sh);
  }
  ...
  if (f->comm != MPI_COMM_NULL && (!(f->semantics & FVMC_FILE_NO_MPI_IO))) {
                    ...
                    fvmc_gnum_t   global_num_end)
{
  ...
  const fvmc_gnum_t _global_num_start = (global_num_start-1)*stride + 1;
  const fvmc_gnum_t _global_num_end = (global_num_end-1)*stride + 1;

  if (f->semantics & FVMC_FILE_NO_MPI_IO)
                     ...
                     fvmc_gnum_t   global_num_end)
{
  ...
  if (   (f->swap_endian == true && size > 1)
    ...
    BFTC_MALLOC(copybuf, bufsize, unsigned char);

    memcpy(copybuf, buf, bufsize);

    retval = fvmc_file_write_block_buffer(f,
                                         copybuf,
                                         size,
                                         stride,
                                         global_num_start,
              ...
              fvmc_file_seek_t   whence)
{
  ...
  if (f->semantics & FVMC_FILE_NO_MPI_IO) {
    if (f->rank == 0)
      retval = bftc_file_seek(f->sh, (bftc_file_off_t)offset, _bftc_seek[whence]);
  }
    ...
    switch(whence) {
    case FVMC_FILE_SEEK_SET:
      f->offset = offset;
      break;
    case FVMC_FILE_SEEK_CUR:
      f->offset += offset;
      break;
    case FVMC_FILE_SEEK_END:
      {
        MPI_Offset f_size = 0;
    ...
    if (f->semantics & FVMC_FILE_INDIVIDUAL_POINTERS)
...
{
  fvmc_file_off_t retval = 0;

  if (f->semantics & FVMC_FILE_NO_MPI_IO) {

    if (f->rank == 0)
      retval = bftc_file_tell(f->sh);

#if defined(FVMC_HAVE_MPI)
    if (f->comm != MPI_COMM_NULL) {
      int64_t _offset = retval;
      MPI_Bcast(&_offset, 1, fvmc_datatype_to_mpi[FVMC_INT64], 0, f->comm);
      retval = _offset;
    }
#endif
    ...
    if (f->semantics & FVMC_FILE_EXPLICIT_OFFSETS)
      ...
      MPI_Datatype offset_type = fvmc_datatype_to_mpi[FVMC_INT64];
...
fvmc_file_set_default_semantics(fvmc_file_hints_t  hints)
{
  ...
  const fvmc_file_hints_t mpi_io_hints = (  FVMC_FILE_EXPLICIT_OFFSETS
                                         | FVMC_FILE_INDIVIDUAL_POINTERS);

#if defined(FVMC_HAVE_MPI_IO)
  if (   (hints & FVMC_FILE_EXPLICIT_OFFSETS)
      && (hints & FVMC_FILE_INDIVIDUAL_POINTERS))
    retval = 1;
  else if (   (hints & mpi_io_hints)
           && (hints & FVMC_FILE_NO_MPI_IO))
    retval = 1;
#else
...
{
  const char *mode_name[] = {"FVMC_FILE_MODE_READ",
                             "FVMC_FILE_MODE_WRITE",
                             "FVMC_FILE_MODE_APPEND"};
  ...
  bftc_printf("\n"
             ...
             (f->semantics & FVMC_FILE_NO_MPI_IO),
             (f->semantics & FVMC_FILE_NO_PREDISTRIBUTE) >> 1,
             (f->semantics & FVMC_FILE_EXPLICIT_OFFSETS) >> 2,
             (f->semantics & FVMC_FILE_INDIVIDUAL_POINTERS) >> 3,
             f->rank, f->n_ranks, (int)(f->swap_endian),
             f->sh);

#if defined(FVMC_HAVE_MPI)
  bftc_printf("Associated communicator:  %lu\n",
             (unsigned long)(f->comm));
#if defined(FVMC_HAVE_MPI_IO)
  bftc_printf("MPI file handle:          %lu\n"
               "MPI file offset:          %lu\n",
             (unsigned long)(f->fh),
