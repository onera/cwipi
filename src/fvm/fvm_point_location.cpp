...
_check_multi_point(const int           dim,
                   const fvm_lnum_t    n_point,
                   const fvm_coord_t   point_coords[],
                   const fvm_lnum_t    point_idx[])
{
  fvm_lnum_t i, j, k;
  double dist;
                   ...
                   float                distance[])
{
  ...
  if (len2 < _epsilon_denom){
    bft_printf("warning _locate_on_edge_3d : Reduce _epsilon_denom criteria : %12.5e < %12.5e\n", len2, _epsilon_denom);
    bft_printf_flush();
    return;
  }
                   ...
                   float                distance[])
{
  ...
  if (len2 < _epsilon_denom){
    bft_printf("warning _locate_on_edge_2d : Reduce _epsilon_denom criteria : %12.5e < %12.5e\n", len2, _epsilon_denom);
    bft_printf_flush();
    return;
  }
                        ...
                        float                distance[])
{
  ...
  for (tria_id = 0; tria_id < n_triangles; tria_id++) {
    ...
    if (det < _epsilon_denom){
      bft_printf("warning _locate_on_triangles_2d : Reduce _epsilon_denom criteria : %12.5e < %12.5e\n", det, _epsilon_denom);
      bft_printf_flush();
      continue;
    }
                 ...
                 float              distance[])
{
  ...
  if (vol6 < _epsilon_denom){
    bft_printf("warning _locate_in_tetra : Reduce _epsilon_denom criteria : %12.5e < %12.5e\n", vol6, _epsilon_denom);
    bft_printf_flush();
    return;
  }
