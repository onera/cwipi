...
typedef struct {
  ...
  fvmc_to_ensight_case_t  *case_info;  /* Associated case structure */

#if defined(FVMC_HAVE_MPI)
  MPI_Comm     comm;             /* Associated MPI communicator */
#endif

} fvmc_to_ensight_writer_t;

/*============================================================================
 * Static global variables
 *============================================================================*/

static const char  *_ensight_type_name[FVMC_N_ELEMENT_TYPES] = {"bar2",
                                                               "tria3",
                                                               "quad4",
                                                               "nsided",
                                                               "tetra4",
...
static void
_count_extra_vertices(const fvmc_to_ensight_writer_t  *this_writer,
...
static void
_write_string(bftc_file_t  *f,
              const char  *s)
{
  size_t  i;
  char  buf[82];
  bftc_file_type_t  type;

  /* If called by non I/O rank, return */
  if (f == NULL)
    return;

  type = bftc_file_get_type(f);

  if (type == BFTC_FILE_TYPE_TEXT) {
    strncpy(buf, s, 80);
    buf[80] = '\0';
           ...
           const int    n)
{
  ...
  if (bftc_file_get_type(f) == BFTC_FILE_TYPE_TEXT)
    bftc_file_printf(f, "%10d\n", n);

  else
    bftc_file_write(&n, sizeof(int), 1, f);
}
...
static void
_init_geom_file(fvmc_to_ensight_writer_t  *this_writer)
{
  if (this_writer->rank == 0) {

    bftc_file_t *f;
    bftc_file_type_t file_type;
    fvmc_to_ensight_case_file_info_t  file_info;

    if (this_writer->text_mode == true)
      file_type = BFTC_FILE_TYPE_TEXT;
    else
      file_type = BFTC_FILE_TYPE_BINARY;

    /* Write file header (future calls will be in append mode) */

    file_info = fvmc_to_ensight_case_get_geom_file(this_writer->case_info);

    f = bftc_file_open(file_info.name,
                      BFTC_FILE_MODE_WRITE,
                      file_type);

    if (this_writer->swap_endian == true)
      bftc_file_set_swap_endian(f, 1);
    ...
    _write_string(f, "Output by FVM library version "FVMC_VERSION);
...
static void
_write_slice_values(fvmc_gnum_t    num_start,
                    fvmc_gnum_t    num_end,
                    const float   values[],
                    bftc_file_t   *f)
{
  ...
  if (bftc_file_get_type(f) == BFTC_FILE_TYPE_TEXT) {
    for (i = 0, j = num_start ; j < num_end ; i++, j++)
      bftc_file_printf(f, "%12.5e\n", values[i]);
  }
  else
    bftc_file_write(values, sizeof(float), num_end - num_start, f);
}
...
static fvmc_coord_t *
_extra_vertex_coords(const fvmc_to_ensight_writer_t  *this_writer,
                     const fvmc_nodal_t              *mesh)
{
  int  i;
...
static void
_export_vertex_coords_g(const fvmc_to_ensight_writer_t  *this_writer,
                        const fvmc_nodal_t              *mesh,
                        MPI_Comm                        comm,
                        ...
                        bftc_file_t                     *f)
{
  ...
  const fvmc_lnum_t  n_vertices
                      = fvmc_io_num_get_local_count(mesh->global_vertex_num);
  ...
  BFTC_MALLOC(coords_tmp, FVMC_MAX(n_vertices, n_extra_vertices), float);
  BFTC_MALLOC(global_coords_s, global_s_size, float);
...
static void
_export_vertex_coords_l(const fvmc_to_ensight_writer_t  *this_writer,
                        const fvmc_nodal_t              *mesh,
                        bftc_file_t                     *f)
{
  fvmc_lnum_t   i, j;
  ...
  for (j = 0 ; j < 3 ; j++) {
    ...
    if (n_extra_vertices > 0)
      _write_slice_values(1,
                          (fvmc_gnum_t)(n_extra_vertices + 1),
                          coords_tmp,
                          f);
...
static void
_export_point_elements_l(fvmc_gnum_t   n_g_vertices,
                         size_t       buffer_size,
                         int          buffer[],
                         bftc_file_t  *f)
{
  ...
  if (bftc_file_get_type(f) == BFTC_FILE_TYPE_TEXT) { /* Text mode */
    ...
    while (j < j_end) {
      for (k = 0 ;  j < j_end  && k < buffer_size ; k++)
        buffer[k] = j++;
      bftc_file_write(buffer, sizeof(int), k, f);
    }
                       ...
                       bftc_file_t        *f)
{
  ...
  if (bftc_file_get_type(f) == BFTC_FILE_TYPE_TEXT) { /* Text mode */
    ...
    case 3: /* FVMC_FACE_TRIA */
      for (i = 0, j = num_start ; j < num_end ; i++, j++)
        bftc_file_printf(f, "%10d%10d%10d\n",
    ...
    case 4: /* FVMC_FACE_QUAD or FVMC_CELL_TETRA */
      for (i = 0, j = num_start ; j < num_end ; i++, j++)
        bftc_file_printf(f, "%10d%10d%10d%10d\n",
    ...
    case 5: /* FVMC_CELL_PYRAM */
      for (i = 0, j = num_start ; j < num_end ; i++, j++)
        bftc_file_printf(f, "%10d%10d%10d%10d%10d\n",
    ...
    case 6: /* FVMC_CELL_PRISM */
      for (i = 0, j = num_start ; j < num_end ; i++, j++)
        bftc_file_printf(f, "%10d%10d%10d%10d%10d%10d\n",
    ...
    while (k < n_values) {
      ...
      bftc_file_write(buffer, sizeof(int), i, f);
    }
                 ...
                 bftc_file_t        *f)
{
  ...
  if (bftc_file_get_type(f) == BFTC_FILE_TYPE_TEXT) { /* Text mode */
    ...
    while (k < n_values) {
      ...
      bftc_file_write(buffer, sizeof(int), j, f);
    }
...
static void
_write_lengths_g(const fvmc_io_num_t  *global_element_num,
                 const fvmc_lnum_t     vertex_index[],
                 MPI_Comm             comm,
                 fvmc_gnum_t           global_s_size,
                 fvmc_gnum_t           global_connect_s_size_caller,
                 ...
                 bftc_file_t          *f)
{
  ...
  const fvmc_lnum_t  n_elements = fvmc_io_num_get_local_count(global_element_num);
  ...
  BFTC_MALLOC(lengths, n_elements, fvmc_lnum_t);
  ...
  if (  sizeof(fvmc_lnum_t)*global_s_size
      > sizeof(fvmc_gnum_t)*global_connect_s_size) {
    global_connect_s_size =   (global_s_size*sizeof(fvmc_lnum_t))
                            / sizeof(fvmc_gnum_t);
  ...
  elements_slice = fvmc_gather_slice_create(global_element_num,
                                           global_s_size,
                                           comm);
      ...
      if (text_mode == true) {
        ...
        for (i = 0, i_s = global_num_start, i_buf = 0 ;
          ...
          if (i_buf == buffer_size) {
            bftc_file_write(buffer, sizeof(int), i_buf, f);
            i_buf = 0;
          }
...
static void
_write_indexed_connect_g(const fvmc_io_num_t  *global_vertex_num,
                         ...
                         bftc_file_t          *f)
{
  ...
  while (fvmc_gather_slice_advance(elements_slice,
    ...
    fvmc_gather_slice_index(vertex_index,
                           global_idx_s,
                           global_element_num,
                           comm,
                           elements_slice);
    ...
    fvmc_gather_indexed_numbers(vertex_index,
                               vertex_num,
                               global_connect_s,
                               global_vertex_num,
                               global_element_num,
  ...
  elements_slice = fvmc_gather_slice_destroy(elements_slice);
...
static void
_write_tesselated_connect_g(const fvmc_io_num_t  *global_vertex_num,
                            const fvmc_io_num_t  *global_element_num,
                            const fvmc_tesselation_t  *tesselation,
                            fvmc_element_t        type,
                            const fvmc_gnum_t     extra_vertex_base,
                            MPI_Comm             comm,
                            fvmc_gnum_t           global_s_size,
                            fvmc_gnum_t           global_connect_s_size_caller,
                            fvmc_gnum_t           global_connect_s_caller[],
                            ...
                            bftc_file_t          *f)
{
  ...
  fvmc_lnum_t  n_sub_elements_max, local_connect_size;
  fvmc_gnum_t  global_num_start, global_num_end;
  ...
  while (fvmc_gather_slice_advance(elements_slice,
    ...
    if (rank == 0) {
      fvmc_gnum_t num_start = global_idx_s[0] / stride;
      fvmc_gnum_t num_end = global_idx_s[(  global_num_end
...
_export_nodal_polyhedra_g(const fvmc_writer_section_t  *export_section,
                          ...
                          bftc_file_t         *f)
{
  ...
  fvmc_lnum_t  n_face_lengths_max = 0;

  fvmc_lnum_t  *face_lengths = NULL;
  fvmc_lnum_t  *cell_vtx_idx = NULL;
  fvmc_lnum_t  *cell_vtx_num = NULL;
  fvmc_gnum_t  *global_idx_s = NULL;
  ...
  const fvmc_writer_section_t  *current_section;
  ...
  do { /* loop on sections which should be appended */
    ...
    const fvmc_nodal_section_t  *section = current_section->section;
    const fvmc_io_num_t  *_global_vertex_num = global_vertex_num;

    BFTC_MALLOC(cell_vtx_idx, section->n_elements + 1, fvmc_lnum_t);
    ...
    BFTC_MALLOC(cell_vtx_num, cell_vtx_idx[section->n_elements], fvmc_lnum_t);
    ...
    /* In text mode, we must apply global vertex numberings here so as to add
       ...
       converted to global numbers by fvmc_gather_...() functions */

    if (text_mode == true) {
      if (global_vertex_num != NULL) {
        const fvmc_gnum_t * g_v_num
...
static const fvmc_writer_section_t *
_export_nodal_polyhedra_l(const fvmc_writer_section_t  *export_section,
                          _Bool                        text_mode,
                          size_t                       buffer_size,
                          int                          buffer[],
                          bftc_file_t                  *f)
      ...
      for (j = section->face_index[i] ;
        ...
        if (text_mode == true) {
          for (k = 0 ; k < face_length ; k++) {
            ...
            bftc_file_printf(f, "%10d", (int)section->vertex_num[l]);
          }
          bftc_file_printf(f, "\n");
        }
        else { /* text_mode == false */
    ...
    for (i = 0, i_buf = 0 ; i < section->n_elements ; i++) {
      ...
      if (text_mode == true) {
        for (j = section->vertex_index[i] ;
             ...
             j++)
          bftc_file_printf(f, "%10d", (int)section->vertex_num[j]);
        bftc_file_printf(f, "\n");
      }
      else { /* text_mode = false */
        for (j = section->vertex_index[i] ;
                           ...
                           bftc_file_t                  *f)
{
  ...
  do { /* loop on sections which should be appended */
    ...
    fvmc_lnum_t  start_id, end_id;
    fvmc_lnum_t  n_sub_elements_max;
    fvmc_lnum_t  n_buffer_elements_max = section->n_elements;
...
#if defined(FVMC_HAVE_MPI)
void *
fvmc_to_ensight_init_writer(const char             *name,
                           const char             *path,
                           const char             *options,
...
#endif
{
  fvmc_to_ensight_writer_t  *this_writer = NULL;

  /* Initialize writer */

  BFTC_MALLOC(this_writer, 1, fvmc_to_ensight_writer_t);
  ...
  this_writer->case_info = fvmc_to_ensight_case_create(name,
                                                      path,
                                                      time_dependency);

  /* Initialize geometry file name */

  if (time_dependency == FVMC_WRITER_FIXED_MESH)
...
void *
fvmc_to_ensight_finalize_writer(void  *this_writer_p)
{
  fvmc_to_ensight_writer_t  *this_writer
                             = (fvmc_to_ensight_writer_t *)this_writer_p;

  BFTC_FREE(this_writer->name);

  fvmc_to_ensight_case_destroy(this_writer->case_info);
...
void
fvmc_to_ensight_set_mesh_time(void          *this_writer_p,
                             const int      time_step,
                             const double   time_value)
{
  ...
  fvmc_to_ensight_case_set_geom_time(this_writer->case_info,
                                    time_step,
                                    time_value);
}
...
int
fvmc_to_ensight_needs_tesselation(fvmc_writer_t       *this_writer_p,
                                 const fvmc_nodal_t  *mesh,
...
void
fvmc_to_ensight_export_nodal(void               *this_writer_p,
                            const fvmc_nodal_t  *mesh)
{
  int     part_num;
  ...
  part_num = fvmc_to_ensight_case_get_part_num(this_writer->case_info,
                                              mesh->name);
  if (part_num == 0)
    part_num = fvmc_to_ensight_case_add_part(this_writer->case_info,
                                            mesh->name);
  ...
  if (rank == 0) {
    if (this_writer->text_mode == true) {
      ...
      BFTC_MALLOC(write_connect_buffer, write_connect_buffer_size, int);
    }
  }
  ...
  while (export_section != NULL) {
    ...
    if (export_section->continues_previous == false && rank == 0) {
      ...
      const fvmc_writer_section_t  *next_section = export_section;

      do {

        if (next_section->section->type == export_section->type)
          n_g_elements += fvmc_nodal_section_n_g_elements(next_section->section);

        else {
          fvmc_gnum_t n_g_sub_elements = 0;
          fvmc_tesselation_get_global_size(next_section->section->tesselation,
                                          next_section->type,
                                          &n_g_sub_elements,
                                          NULL);
    ...
    else if (export_section->type == FVMC_CELL_POLY) {
  ...
  if (write_connect_buffer != NULL) {
    write_connect_buffer_size = 0;
    BFTC_FREE(write_connect_buffer);
  }
  ...
  if (rank == 0)
    fvmc_to_ensight_case_write_case(this_writer->case_info);
}
...
void
fvmc_to_ensight_export_field(void                  *this_writer_p,
                            const fvmc_nodal_t     *mesh,
                            const char            *name,
                            ...
                            const void      *const field_values[])
{
  ...
  if (rank == 0) {
    ...
    file_info = fvmc_to_ensight_case_get_var_file(this_writer->case_info,
                                                 name,
                                                 output_dim,
                                                 location,
                                                 time_step,
    ...
    if (file_info.queried == true)
      file_mode = BFTC_FILE_MODE_APPEND;
    else
      file_mode = BFTC_FILE_MODE_WRITE;
    ...
    if (file_mode == BFTC_FILE_MODE_WRITE){
      char buf[81] = "";
#if HAVE_SNPRINTF
      if (time_step > -1)
  ...
  if (n_ranks > 1) {
    size_t min_buffer_size = fvmc_parall_get_min_coll_buf_size() / sizeof(float);
  ...
  BFTC_MALLOC(var_buffer, var_buffer_size, float);
