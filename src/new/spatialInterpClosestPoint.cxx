...
// //        cout << cplComm_rank << ": PDM_closest_point freed " << _id_pdm << endl;
      ...
      // not 100% confident...
      if (_ptsp == NULL) {
        // PDM_error(__FILE__, __LINE__, 0, "_ptsp is NULL");
        ...
        if (!_coupledCodeProperties->localCodeIs()) {
          ...
          // int **tgt_to_src_idx = (int **) malloc(sizeof(int *) * _nPart);
          // for (int i_part = 0; i_part < _nPart; i_part++) {
          //   tgt_to_src_idx[i_part] = PDM_array_new_idx_from_const_stride_int(n_closest_pts,
          //                                                                    _tgt_n_gnum[i_part]);
          //   if (1) {
          //     PDM_log_trace_connectivity_long(tgt_to_src_idx[i_part],
          //                                     _closest_src_gnum[i_part],
          //                                     _tgt_n_gnum[i_part],
          //                                     "_closest_src_gnum : ");
          //   }
          ...
          // /* if part1 : tgt, part2 : src then we need to perform reverse exchanges... */
          // /* else we create ptp from source->tgt */
          ...
          //                                 (const PDM_g_num_t **) _closest_src_gnum,
          //                                 _pdmCplComm);

          // for (int i_part = 0; i_part < _nPart; i_part++) {
          //   free(tgt_to_src_idx[i_part]);
          // }
          // free(tgt_to_src_idx);
            ...
            if (_ptsp == NULL) {
              if (_exchDirection == SPATIAL_INTERP_EXCH_SEND) {
                _ptsp = PDM_part_to_part_create ((const PDM_g_num_t **)_src_gnum,
                                             ...
                                             (const PDM_g_num_t **)_tgt_in_src_gnum,
                                             _pdmCplComm);
                // int **tgt_to_src_idx = (int **) malloc(sizeof(int *) * _cplNPart);
                // for (int i_part = 0; i_part < _cplNPart; i_part++) {
                //   tgt_to_src_idx[i_part] = PDM_array_new_idx_from_const_stride_int(n_closest_pts,
                //                                                                    cpl_spatial_interp->_tgt_n_gnum[i_part]);
                //   if (1) {
                //     PDM_log_trace_connectivity_long(tgt_to_src_idx[i_part],
                //                                     _closest_src_gnum[i_part],
                //                                     _tgt_n_gnum[i_part],
                //                                     "cpl_spatial_interp->_closest_src_gnum : ");
                //   }
                // }
                ...
                //                                 _nPart,
                //                                 (const int         **) tgt_to_src_idx,
                //                                 (const PDM_g_num_t **) cpl_spatial_interp->_closest_src_gnum,
                //                                 _pdmCplComm);
