...
namespace cwipi {
    ...
    void SpatialInterpLocation::spatialInterpWeightsCompute(CWP_Field_exch_t Texch_t) {
        ...
        if ((!_both_codes_are_local && _cpl->commTypeGet() == CWP_COMM_PAR_WITH_PART && _isCoupledRank)
            ...
            if (_Texch_t == CWP_FIELD_EXCH_SEND) initialization_of_receving_communication_tree_array();
            if (_Texch_t == CWP_FIELD_EXCH_RECV) triplet_location_null_recv();
            if (_Texch_t == CWP_FIELD_EXCH_SEND) triplet_location_null_send();
    ...
    void *SpatialInterpLocation::interpolate(Field *referenceField) {
        ...
        if (interpolatedData != nullptr) free(interpolatedData);
        interpolatedData = malloc(dataTypeSize * nComponent * _n_tot_target_cpl);

        if (_weights_src_idx == nullptr) {
            _weights_src_idx = (int **) malloc(sizeof(int *) * _nb_part);
            for (int i = 0 ; i < _nb_part ; i++) _weights_src_idx[i] = nullptr;
            _weights_src = (double **) malloc(sizeof(double *) * _nb_part);
            for (int i = 0 ; i < _nb_part ; i++) _weights_src[i] = nullptr;
        }
        ...
        for (int i_part = 0 ; i_part < _nb_part ; i_part++) {
            ...
            if (_weights_src_idx[i_part] == nullptr) {
            ...
            for (int i_proc = 0 ; i_proc < cplComm_size ; i_proc++) {
                ...
                if (interpolationType == CWP_INTERPOLATION_USER) {
                    ...
                    int *tgt_pts_bary_coords_idx = nullptr;
                    double *tgt_pts_bary_coords = nullptr;
                    ...
                    if (tgt_pts_location != nullptr) free(tgt_pts_location);
                    if (tgt_pts_location_p1 != nullptr) free(tgt_pts_location_p1);
                    if (tgt_pts_dist != nullptr) free(tgt_pts_dist);
                    if (tgt_pts_projected_coords != nullptr) free(tgt_pts_projected_coords);
                    if (tgt_pts_bary_coords_idx != nullptr) free(tgt_pts_bary_coords_idx);
                    if (tgt_pts_bary_coords != nullptr) free(tgt_pts_bary_coords);
                }
                        ...
                        for (int itarget = _targets_localization_idx_cpl[i_proc][i_part] ; itarget < _targets_localization_idx_cpl[i_proc][i_part + 1] ; itarget++) {
                            ...
                            if (_targets_localization_data_cpl[itarget].distance != INFINITY) {
                                ...
                                if (weights_src_empty) {
                                    int *_barCoordsIndex = nullptr;
                                    double *_barCoords = nullptr;
    ...
    **********************************************************/
    void SpatialInterpLocation::triplet_location_request() {
        ...
        for (int i_part = 0 ; i_part < _nb_part ; i_part++) PDM_gnum_location_requested_elements_set(_id_gnum_location, i_part, _n_target[i_part], &(_closest_elt_gnum[i_part][0]));

        if (!_both_codes_are_local)
            for (int i_part = 0 ; i_part < _nb_part_cpl ; i_part++) PDM_gnum_location_elements_set(_id_gnum_location, i_part, 0, nullptr);
        else {
    ...
    void SpatialInterpLocation::triplet_location_set() {
        ...
        if (!_both_codes_are_local)
            for (int i_part = 0 ; i_part < _nb_part_cpl ; i_part++) PDM_gnum_location_requested_elements_set(_id_gnum_location, i_part, 0, nullptr);
        else {
            for (int i_part = 0 ; i_part < _nb_part_cpl ; i_part++) {
    ...
    void SpatialInterpLocation::triplet_location_null_send() {
        ...
        for (int i_part = 0 ; i_part < _nb_part ; i_part++) PDM_gnum_location_elements_set(_id_gnum_location, i_part, 0, nullptr);
        for (int i_part = 0 ; i_part < _nb_part_cpl ; i_part++) PDM_gnum_location_requested_elements_set(_id_gnum_location, i_part, 0, nullptr);
    }
    ...
    void SpatialInterpLocation::triplet_location_null_recv() {
        ...
        for (int i_part = 0 ; i_part < _nb_part_cpl ; i_part++) PDM_gnum_location_elements_set(_id_gnum_location, i_part, 0, nullptr);
        for (int i_part = 0 ; i_part < _nb_part ; i_part++) PDM_gnum_location_requested_elements_set(_id_gnum_location, i_part, 0, nullptr);
    }
    ...
    void SpatialInterpLocation::triplet_location_get() {
        ...
        for (int i_part = 0 ; i_part < _nb_part ; i_part++) PDM_gnum_location_get(_id_gnum_location, i_part, &(_target_proc_part_num_idx[i_part]), &(_target_proc_part_num[i_part]));
    }
    ...
    void SpatialInterpLocation::triplet_location_get_cpl() {
        ...
        for (int i_part = 0 ; i_part < _nb_part_cpl ; i_part++) PDM_gnum_location_get(_id_gnum_location, i_part, &(_spatial_interp_cpl->_target_proc_part_num_idx[i_part]), &(_spatial_interp_cpl->_target_proc_part_num[i_part]));
    }
    ...
    **********************************************************/
    void SpatialInterpLocation::initialization_of_receving_communication_tree_array() {
        if (_targets_localization_idx_cpl == nullptr) {
            _targets_localization_idx_cpl = (int **) malloc(sizeof(int *) * cplComm_size);
            for (int i = 0 ; i < cplComm_size ; i++) _targets_localization_idx_cpl[i] = nullptr;
        }

        for (int i = 0 ; i < cplComm_size ; i++) {
            if (_targets_localization_idx_cpl[i] == nullptr) _targets_localization_idx_cpl[i] = (int *) malloc(sizeof(int) * (1 + _nb_part));
            for (int i_part = 0 ; i_part < _nb_part + 1 ; i_part++) _targets_localization_idx_cpl[i][i_part] = 0;
        }
    }
    ...
    void SpatialInterpLocation::filling_of_sending_communication_tree_array() {
        ...
        for (int i_proc = 0 ; i_proc < cplComm_size ; i_proc++) _targets_localization_idx[i_proc] = nullptr;
        ...
        for (int i_proc = 0 ; i_proc < cplComm_size ; i_proc++) {
            ...
            for (int i_part = 0 ; i_part < _nb_part_cpl ; i_part++) idx_proc[i_proc][i_part] = 0;
        }

        if (_targets_localization_data != nullptr) free(_targets_localization_data);
        ...
        for (int i_proc = 0 ; i_proc < cplComm_size ; i_proc++) free(idx_proc[i_proc]);
