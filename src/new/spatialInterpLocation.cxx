...
namespace cwipi {
    ...
    **********************************************************/
    void SpatialInterpLocation::triplet_location_request() {
        ...
        if (!_both_codes_are_local)
            for (int i_part = 0 ; i_part < _nb_part_cpl ; i_part++) PDM_gnum_location_elements_set(_id_gnum_location, i_part, 0, NULL);
        else {
    ...
    void SpatialInterpLocation::triplet_location_null_send() {
        ...
        for (int i_part = 0 ; i_part < _nb_part ; i_part++) PDM_gnum_location_elements_set(_id_gnum_location, i_part, 0, NULL);
        for (int i_part = 0 ; i_part < _nb_part_cpl ; i_part++) PDM_gnum_location_requested_elements_set(_id_gnum_location, i_part, 0, NULL);
    }
    ...
    void SpatialInterpLocation::triplet_location_null_recv() {
        ...
        for (int i_part = 0 ; i_part < _nb_part_cpl ; i_part++) PDM_gnum_location_elements_set(_id_gnum_location, i_part, 0, NULL);
        for (int i_part = 0 ; i_part < _nb_part ; i_part++) PDM_gnum_location_requested_elements_set(_id_gnum_location, i_part, 0, NULL);
    }
    ...
    **********************************************************/
    void SpatialInterpLocation::initialization_of_receving_communication_tree_array() {
        if (_targets_localization_idx_cpl == NULL) {
            ...
            for (int i = 0 ; i < cplComm_size ; i++) _targets_localization_idx_cpl[i] = NULL;
        }

        for (int i = 0 ; i < cplComm_size ; i++) {
            if (_targets_localization_idx_cpl[i] == NULL) _targets_localization_idx_cpl[i] = (int *) malloc(sizeof(int) * (1 + _nb_part));
