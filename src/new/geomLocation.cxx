...
namespace cwipi {
  ...
  void GeomLocation::user_target_points_set(int i_part, int n_pts, double* coord) {
    if( !(_geometryLocation == CWP_FIELD_VALUE_USER ) )
  ...
  void GeomLocation::user_targets_gnum_compute() {
    int coord_def = 1;
...
/***************************************************************************/
  ...
  void GeomLocation::compute(CWP_Field_exch_t Texch_t) {
    _Texch_t = Texch_t;  
    
    if(_both_codes_are_local == 0){
      if(_Texch_t == CWP_FIELD_EXCH_RECV && _geometryLocation == CWP_FIELD_VALUE_USER)  
        user_targets_gnum_compute();
    }
    else {
      if(_Texch_t == CWP_FIELD_EXCH_SEND) {
        _geometry_cpl -> _Texch_t =  CWP_FIELD_EXCH_RECV;
        if(_isCoupledRank && _geometryLocation == CWP_FIELD_VALUE_USER) 
          _geometry_cpl -> user_targets_gnum_compute();
      }
    }  
    ...
    if(_both_codes_are_local == 0){
      if(_isCoupledRank) {   
        ...
        if(_Texch_t == CWP_FIELD_EXCH_RECV ) {
          localization_points_cloud_setting(&_id_dist);
        }
  ...
  void GeomLocation::mesh_info_get() {
    ...
    for(int i_part =0;i_part<_nb_part;i_part++) {   
      if (_geometryLocation == CWP_FIELD_VALUE_CELL_POINT && _Texch_t == CWP_FIELD_EXCH_RECV ) {
      ...
      else if (_geometryLocation == CWP_FIELD_VALUE_NODE && _Texch_t == CWP_FIELD_EXCH_RECV) {
...
void GeomLocation::mesh_cpl_info_get() {
  ...
  void GeomLocation::init(Coupling *coupling, CWP_Field_value_t geometryLocation, int slave) {
   ...
   */
    ...
    if(_both_codes_are_local == 1 && slave == 0) {
      ...
      _geometry_cpl = dynamic_cast<GeomLocation*>( coupling_cpl.geometryGet(_geometryLocation ) );
...
void GeomLocation::issend(Field* referenceField) {
    ...
    printf("ISSEND %s referenceField -> fieldIDIntGet() %i rank %i both %i request %i\n",
    referenceField ->fieldIDGet().c_str(),referenceField -> fieldIDIntGet(),_rank,_both_codes_are_local,request);
...
void GeomLocation::null_exchange_for_uncoupled_process() {
  ...
  void GeomLocation::both_codes_on_the_same_process_exchange(Field* referenceField,Field* recevingField) {
     ...
     _idx_target  .resize   (_geometry_cpl -> _nb_part + 1);
     _idx_target[0] = 0;
     for (int i_part = 0; i_part < _geometry_cpl -> _nb_part; i_part++) {
       _idx_target[i_part+1] = _idx_target[i_part] + _geometry_cpl -> _n_target[i_part];   
     }
     ...
     for (int i_proc=0; i_proc < _n_ranks_g; i_proc++) {
       count_recv[i_proc]  =  dataTypeSize_recv * nComponent_recv 
                           * ( _geometry_cpl -> _targets_localization_idx[ i_proc ][_geometry_cpl -> _nb_part_cpl] - _geometry_cpl -> _targets_localization_idx[i_proc][0]  );
       displ_recv [i_proc]  =  dataTypeSize_recv * nComponent_recv * _geometry_cpl -> _targets_localization_idx[i_proc][0];
     }
  ...
  void GeomLocation::localization_surface_setting(int* id_dist) {
...
void GeomLocation::localization_null_setting(int* id_dist) {
  ...
  void GeomLocation::localization_points_cloud_setting(int* id_dist) {
  ...
  void GeomLocation::localization_compute(int id_dist) {
  ...
  *id_gnum_location = PDM_gnum_location_create(_nb_part,_nb_part_cpl, _pdm_unionComm);
  ...
  if(_both_codes_are_local == 0) {
   ...
   /*        printf("CPL_closest_elt_gnum[i_part][%i] rank %i %I64d %I64d coords %f %f %f _distance %f N %i\n",
       ...
       PDM_gnum_location_requested_elements_set(*id_gnum_location,i_part, n_target_cpl, _geometry_cpl -> _closest_elt_gnum[i_part]);
   }//loop on part
  }//end if
 }
 /*************************************************************************************/
...
void GeomLocation::broadcasting_set_null(int* id_gnum_location) {
  ...
  *id_gnum_location = PDM_gnum_location_create(_nb_part,_nb_part_cpl, _pdm_unionComm);
  ...
  void GeomLocation::localization_get_cpl(int id_dist) {
  ...
  void GeomLocation::localization_get(int id_dist) {
 ...
 void GeomLocation::both_index_communication() {
   ...
   for(int i_proc=0;i_proc<_n_ranks_g;i_proc++)
     for(int i_part=0;i_part< _nb_part; i_part++) {
       sbuffer[ i_proc * _nb_part + i_part ] = (_geometry_cpl -> _process_and_partition_count)[i_proc][i_part];
     }
 ...
 void GeomLocation::data_communication_send() {
   ...
   free(disp_recv );
  /*
  _IAlltoallIndexSend((void*)_targets_localization_data, _targets_localization_data_count_send, _targets_localization_data_disp_send,
                  MPI_BYTE,
 ...
 void GeomLocation::both_data_communication() {
   ...
   MPI_Request req;   
   MPI_Ialltoallv((void*) (_geometry_cpl -> _targets_localization_data), _geometry_cpl -> _targets_localization_data_count_send, _geometry_cpl -> _targets_localization_data_disp_send, MPI_BYTE,  
                  (void*)_targets_localization_data_cpl,  _targets_localization_data_count_recv, _targets_localization_data_disp_recv, MPI_BYTE,
                 _unionComm,&req);     
...
/*******************************************************************************************************************/
  ...
  void GeomLocation::irecv(Field *recevingField) {
...
/******************************************************/
  ...
  void GeomLocation::waitIrecv (Field* recevingField) {
    MPI_Status status;
...
/******************************************************/
  ...
  void GeomLocation::waitIssend (Field* sendingField) {
