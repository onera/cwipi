!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!> pyramidRule.f90
!> http://people.sc.fsu.edu/~jburkardt/f_src/pyramid_rule/pyramid_rule.html
!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
module pyramidRule
  ...
  !>                  0 <= Z <= 1.
  !> 
  !>  When Z is zero, the integration region is a square lying in the (X,Y) plane, centered at (0,0,0) with "radius" 1. 
  !>  As Z increases to 1, the radius of the square diminishes, and when Z reaches 1, the square has contracted to the single point (0,0,1).
...
!
  if ( alpha <= -1.0D+00 ) then
    write ( *, '(/"JACOBI_COMPUTE - Fatal error : -1.0 < ALPHA is required"/)')
    stop 1
  endif
  
  if ( beta <= -1.0D+00 ) then
    write ( *, '(/"JACOBI_COMPUTE - Fatal error : -1.0 < BETA is required"/)')
    stop 1
  endif
!
...
!
  ...
  delta = r8_gamma( alpha        + 1d0 ) &
        * r8_gamma(         beta + 1d0 ) &
        / r8_gamma( alpha + beta + 2d0 )
