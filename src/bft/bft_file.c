...
typedef bft_z_off_t (bft_gztell_t) (gzFile file);
...
static bft_gzseek_t  *_bft_gzseek = (bft_gzseek_t *)gzseek;
static bft_gztell_t  *_bft_gztell = (bft_gztell_t *)gztell;
...
# else
    ...
    */
    ...
    offset = (bft_file_off_t)_bft_gztell(f->gzptr);
 ...
 * successful call to ftell() (here bft_file_ftell()) on a stream (here a
 * bft_file_t structure). Zlib's gzseek() does not support SEEK_END, at least
 * as of version 1.2.1.
 *
 * A successful call to this function clears the end-of-file indicator for
 * this file.
...
# endif /* defined(HAVE_FSEEKO) && (_FILE_OFFSET_BITS == 64) */
    ...
    retval = _bft_gzseek(f->gzptr, (bft_z_off_t)offset, _whence);
