 ...
 *                            string.
 *
 * \dontinclude bftc_error_example.c
 *
...
static void
_bftc_error_handler_default(const char     *const file_name,
                           const int             line_num,
...
static bftc_error_handler_t  *_bftc_error_handler = (_bftc_error_handler_default);
 ...
 * Default error handler.
 *
 * An error message is output to stderr (after bftc_print_flush() is called),
...
/*!
 * \brief Calls the error handler (set by bftc_error_handler_set() or default).
 *
 * With the default error handler, bftc_print_flush() is called, an error
...
void
bftc_error(const char  *const file_name,
          const int          line_num,
          const int          sys_error_code,
          const char  *const format,
          ...)
{
  ...
  _bftc_error_handler(file_name, line_num, sys_error_code, format, arg_ptr);

  va_end(arg_ptr);
}

/*!
 * \brief Returns the error handler associated with the bftc_error() function.
 *
 * \return pointer to the error handler function.
 */

bftc_error_handler_t *
bftc_error_handler_get(void)
{
  return _bftc_error_handler;
}

/*!
 * \brief Associates an error handler with the bftc_error() function.
 *
 * \param [in] handler pointer to the error handler function.
 */

void
bftc_error_handler_set(bftc_error_handler_t  *const handler)
{
  _bftc_error_handler = handler;
}
