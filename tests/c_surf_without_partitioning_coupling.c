...
/*   double randLevel = 0.4; */
  ...
  char* fileName = (char *) malloc(sizeof(char) * 47);
  ...
  fprintf(outputFile, "  Test :  CWIPI_COUPLING_PARALLEL_WITHOUT_PARTITIONING/"
  ...
  if (codeId == 1)
    coupling_type = CWIPI_COUPLING_PARALLEL_WITH_PARTITIONING;
  else
    coupling_type = CWIPI_COUPLING_PARALLEL_WITHOUT_PARTITIONING;

  cwipi_create_coupling("c_surf_without_partitioning_cpl",                 // Coupling id
  ...
  if (codeId == 1) {
    ...
    cwipi_define_mesh("c_surf_without_partitioning_cpl",
                      nVertex,
                      nElts,
                      coords,
                      eltsConnecPointer,
  ...
  cwipi_exchange_status_t status = cwipi_exchange("c_surf_without_partitioning_cpl",
                                                  "ech",
                                                  1,
                                                  1,     // n_step
                                                  0.1,   // physical_time
  ...
  _dumpNotLocatedPoints(outputFile, "c_surf_without_partitioning_cpl", nNotLocatedPoints);
  ...
  cwipi_delete_coupling("c_surf_without_partitioning_cpl");
