...
)
{
  ...
  char *config = NULL;
  if (is_code1) {
    config = (char *) "client_new_api_deformable_o/code1/cwp_config_srv.txt";
  }
  else {
    config = (char *) "client_new_api_deformable_o/code2/cwp_config_srv.txt";
  }
  ...
  if (i_rank == 0) {
    char cmd[999];
    system("mkdir -p client_new_api_deformable_o/code1");
    system("mkdir -p client_new_api_deformable_o/code2");
    system("rm -f ./client_new_api_deformable_o/code1/cwp_config_srv.txt");
    system("rm -f ./client_new_api_deformable_o/code2/cwp_config_srv.txt");

    sprintf(cmd, "mpirun -n %d ../bin/cwp_server -cn code1 -p %d %d -c \"client_new_api_deformable_o/code1/cwp_config_srv.txt\" \
                  : -n %d  ../bin/cwp_server -cn code2 -p %d %d -c \"client_new_api_deformable_o/code2/cwp_config_srv.txt\" &",
                  code_n_rank[0], 49100, 49100 + code_n_rank[0] - 1,
                  code_n_rank[1], 49100 + code_n_rank[0], 49100 + code_n_rank[0] + code_n_rank[1] - 1);
    system(cmd);
  }
  ...
  // Create coupling
  const char *cpl_name = "client_new_api_deformable";
