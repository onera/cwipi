...
n_vtx = len(coords)//3
field_data = np.arange(n_vtx*n_components, dtype=np.double)
...
To check if that tolerence was large enougth, the function **n_uncomputed_tgts_get** can be called to retreive the number of unlocated vertices of the coupling interface of `code 1`.
