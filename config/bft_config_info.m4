...
# BFTC_AC_CONFIG_INFO_INIT([OUTPUT FILE NAME])
#------------------------
# Write main config info file header.

AC_DEFUN([BFTC_AC_CONFIG_INFO_INIT], [

# First arg is output file name
if test "$1" = "" ; then
  bftc_ac_config_info="bft-config"
else
  bftc_ac_config_info=$1
fi

outfile="$bftc_ac_config_info"-tmp
...
AC_MSG_NOTICE([initializing $bftc_ac_config_info])

])

# BFTC_AC_CONFIG_INFO_EXTRA([extra config info strings])
#-------------------------
# Write extra info to config info file header.

AC_DEFUN([BFTC_AC_CONFIG_INFO_EXTRA], [

AC_REQUIRE([BFTC_AC_CONFIG_INFO_INIT])dnl
...
# BFTC_AC_CONFIG_INFO_CC([cc], [version], [version_full])
#----------------------
# Write available compiler info to config info file header.

AC_DEFUN([BFTC_AC_CONFIG_INFO_CC], [
...
# BFTC_AC_CONFIG_INFO_VERSION([version])
#------------------------------
# Write version info.

AC_DEFUN([BFTC_AC_CONFIG_INFO_VERSION], [
...
# BFTC_AC_CONFIG_INFO_DIRS([prefix], [exec_prefix], [includedir], [libdir])
#------------------------
# Write install path info.

AC_DEFUN([BFTC_AC_CONFIG_INFO_DIRS], [
...
if test "$5" != "" ; then
  bftc_ac_config_info_scope="${bftc_ac_config_info_scope} $5"
  scope_prefix="$5_"
else
  scope_prefix=""
fi
...
# BFTC_AC_CONFIG_INFO_FINALIZE
#----------------------------
# Write main config info file body (script part).

AC_DEFUN([BFTC_AC_CONFIG_INFO_FINALIZE], [
...
_______EOF
  ...
  for scope in $bftc_ac_config_info_scope ; do
...
if test "$bftc_ac_config_info_scope" != "" ; then
...
AC_MSG_NOTICE([closing $bftc_ac_config_info])

diff $outfile $bftc_ac_config_info > /dev/null 2>&1
if test $? -eq 0 ; then
  AC_MSG_NOTICE([$bftc_ac_config_info is unchanged])
  rm -f $outfile
else
  mv $outfile $bftc_ac_config_info
  chmod +x $bftc_ac_config_info
fi
