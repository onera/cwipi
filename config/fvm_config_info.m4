...
# FVM_AC_CONFIG_INFO_INIT([OUTPUT FILE NAME])
#------------------------
# Write main config info file header.

AC_DEFUN([FVM_AC_CONFIG_INFO_INIT], [

# First arg is output file name
if test "$1" = "" ; then
  fvm_ac_config_info="fvm-config"
else
  fvm_ac_config_info=$1
fi

outfile="$fvm_ac_config_info"-tmp
...
AC_MSG_NOTICE([initializing $fvm_ac_config_info])

])

# FVM_AC_CONFIG_INFO_EXTRA([extra config info strings])
#-------------------------
# Write extra info to config info file header.

AC_DEFUN([FVM_AC_CONFIG_INFO_EXTRA], [

AC_REQUIRE([FVM_AC_CONFIG_INFO_INIT])dnl
...
# FVM_AC_CONFIG_INFO_CC([cc], [version], [version_full])
#----------------------
# Write available compiler info to config info file header.

AC_DEFUN([FVM_AC_CONFIG_INFO_CC], [
...
# FVM_AC_CONFIG_INFO_VERSION([version])
#------------------------------
# Write version info.

AC_DEFUN([FVM_AC_CONFIG_INFO_VERSION], [
...
AC_DEFUN([FVM_AC_CONFIG_INFO_DIRS], [
...
AC_DEFUN([FVM_AC_CONFIG_INFO_FLAGS], [
...
if test "$5" != "" ; then
  fvm_ac_config_info_scope="${fvm_ac_config_info_scope} $5"
  scope_prefix="$5_"
else
  scope_prefix=""
fi
...
AC_DEFUN([FVM_AC_CONFIG_INFO_FINALIZE], [
...
_______EOF
  ...
  for scope in $fvm_ac_config_info_scope ; do
...
if test "$fvm_ac_config_info_scope" != "" ; then
...
AC_MSG_NOTICE([closing $fvm_ac_config_info])

diff $outfile $fvm_ac_config_info > /dev/null 2>&1
if test $? -eq 0 ; then
  AC_MSG_NOTICE([$fvm_ac_config_info is unchanged])
  rm -f $outfile
else
  mv $outfile $fvm_ac_config_info
  chmod +x $fvm_ac_config_info
fi
