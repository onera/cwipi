...
# COUPLINGS_AC_CONFIG_INFO_INIT([OUTPUT FILE NAME])
#------------------------
# Write main config info file header.

AC_DEFUN([COUPLINGS_AC_CONFIG_INFO_INIT], [

# First arg is output file name
if test "$1" = "" ; then
  couplings_ac_config_info="couplings-config"
else
  couplings_ac_config_info=$1
fi

outfile="$couplings_ac_config_info"-tmp
...
AC_MSG_NOTICE([initializing $couplings_ac_config_info])

])

# COUPLINGS_AC_CONFIG_INFO_EXTRA([extra config info strings])
#-------------------------
# Write extra info to config info file header.

AC_DEFUN([COUPLINGS_AC_CONFIG_INFO_EXTRA], [

AC_REQUIRE([COUPLINGS_AC_CONFIG_INFO_INIT])dnl
...
# COUPLINGS_AC_CONFIG_INFO_CC([cc], [version], [version_full])
#----------------------
# Write available compiler info to config info file header.

AC_DEFUN([COUPLINGS_AC_CONFIG_INFO_CC], [
...
# COUPLINGS_AC_CONFIG_INFO_VERSION([version])
#------------------------------
# Write version info.

AC_DEFUN([COUPLINGS_AC_CONFIG_INFO_VERSION], [
...
# COUPLINGS_AC_CONFIG_INFO_DIRS([prefix], [exec_prefix], [includedir], [libdir])
#------------------------
# Write install path info.

AC_DEFUN([COUPLINGS_AC_CONFIG_INFO_DIRS], [
...
# COUPLINGS_AC_CONFIG_INFO_FLAGS([cppflags], [cflags], [ldflags], [libs],
#                          [scope], [scope_comments])
#-------------------------
# Write compiler and linker flags for a given scope (default if scope is empty).

AC_DEFUN([COUPLINGS_AC_CONFIG_INFO_FLAGS], [
...
if test "$5" != "" ; then
  couplings_ac_config_info_scope="${couplings_ac_config_info_scope} $5"
  scope_prefix="$5_"
else
  scope_prefix=""
fi
...
# COUPLINGS_AC_CONFIG_INFO_FINALIZE
#----------------------------
# Write main config info file body (script part).

AC_DEFUN([COUPLINGS_AC_CONFIG_INFO_FINALIZE], [
...
if test "[$]show_help" = "yes"; then
  cat <<EOF
Usage: couplings-config [options] [scope]
...
_______EOF
  ...
  for scope in $couplings_ac_config_info_scope ; do
...
if test "$couplings_ac_config_info_scope" != "" ; then
...
AC_MSG_NOTICE([closing $couplings_ac_config_info])

diff $outfile $couplings_ac_config_info > /dev/null 2>&1
if test $? -eq 0 ; then
  AC_MSG_NOTICE([$couplings_ac_config_info is unchanged])
  rm -f $outfile
else
  mv $outfile $couplings_ac_config_info
  chmod +x $couplings_ac_config_info
fi
